...
def WEAPON_BASE_EFFECTS(part_name: str, emulate_default_effects: bool = False):
    return [
        EffectsGroup(
            ...
            effects=[
                ...
                SetMaxCapacity(
                    partname=part_name,
                    value=Value + NamedReal(name=part_name + "_PART_CAPACITY", value=PartCapacity(name=part_name) * SHIP_WEAPON_DAMAGE_FACTOR),
                ),
                SetMaxSecondaryStat(
                    partname=part_name,
                    value=Value
                    + NamedReal(name=part_name + "_PART_SECONDARY_STAT", value=PartSecondaryStat(name=part_name) * SHIP_WEAPON_DAMAGE_FACTOR),
                ),
            ],
        ),
    ...
    ] if emulate_default_effects else [
