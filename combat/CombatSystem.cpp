    ...
    void CombatRound(AutoresolveInfo& combat_state) {
        ...
        if (combat_info.bout < NUM_COMBAT_ROUNDS) {
            for (auto* attacker : combat_info.objects.findRaw<Ship>(shuffled_attackers)) {
                ...
                for (auto detector_empire_id : combat_info.empire_ids) {
                    ...
                    if (initial_vis >= Visibility::VIS_PARTIAL_VISIBILITY)
                        continue;

                    combat_info.empire_object_visibility[detector_empire_id].Set(attacker->ID(), Visibility::VIS_PARTIAL_VISIBILITY);

                    DebugLogger(combat) << " ... Setting post-attack visability to " << Visibility::VIS_PARTIAL_VISIBILITY;
        ...
        const auto handle_attack_event = [&combat_info, &stealth_change_event](const auto& attack_event) {
            ...
            for (auto detector_empire_id : combat_info.empire_ids) {
                ...
                combat_info.empire_object_visibility[detector_empire_id].Set(attack_event->attacker_id, Visibility::VIS_PARTIAL_VISIBILITY);

                DebugLogger(combat) << " ... Setting post-attack visability to " << Visibility::VIS_PARTIAL_VISIBILITY;

                // record visibility change event due to attack
                stealth_change_event->AddEvent(attack_event->attacker_id, attack_event->target_id, attack_event->attacker_owner_id,
                                               attack_event->target_owner_id, Visibility::VIS_PARTIAL_VISIBILITY);
            }
        };
