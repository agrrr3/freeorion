                           ...
                           SearchDomain search_domain/* = NON_MATCHES*/) const
{
    ...
    if (simple_eval_safe) {
        ...
        EvalImpl(matches, non_matches, search_domain,
                 VisibleToEmpireSimpleMatch(empire_id, parent_context.combat_context.combat_info.empire_object_visibility));
    } else {
        // re-evaluate empire id for each candidate object
        Condition::Eval(parent_context, matches, non_matches, search_domain);
    }
...
bool VisibleToEmpire::Match(const ScriptingContext& local_context) const {
    ...
    return VisibleToEmpireSimpleMatch(empire_id, local_context.combat_context.combat_info.empire_object_visibility)(candidate);
}
