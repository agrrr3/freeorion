...
namespace {
    std::vector<float> WeaponDamageCalcImpl(std::shared_ptr<const Ship> ship, const ShipDesign* design,
        ...
        if (target_ships)
            ...
            retval.emplace_back(fighter_shots / num_bouts); // divide by bouts because fighter calculation is for a full combat, but direct fire for one attack
        return retval;
    }
                                        ...
                                        float DR, bool max, bool include_fighters, bool target_ships = true)
    {
        if (target_ships) {
            ...
            target->GetMeter(MeterType::METER_SHIELD)->SetCurrent(DR);
